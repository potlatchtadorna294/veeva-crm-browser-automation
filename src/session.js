/**
 * session.js — Cookie & localStorage session persistence for Veeva CRM (Veeva Commercial Cloud)
...
async function loadSession(page) {
  ...
  if (session.localStorage && Object.keys(session.localStorage).length > 0) {
    ...
    const url = new URL(session.url || process.env.VEEVA_CRM_URL || 'https://localhost');
